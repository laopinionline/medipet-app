...
(function (root, factory) {
  ...
  var CATEGORIAS_TIENDA = ['alimento', 'accesorios', 'higiene', 'juguetes', 'camas_descanso', 'salud'];
  // Etiqueta legible por categoría (para góndolas/selects; 'salud' nace sin productos — categoría estratégica).
  var CATEGORIA_LABEL = { alimento: 'Alimento', accesorios: 'Accesorios', higiene: 'Higiene', juguetes: 'Juguetes', camas_descanso: 'Camas y descanso', salud: 'Salud' };
  ...
  // ── MODELO DE DOS EJES (Fase 2) — logística (estado) + pago, independientes ──
  // Definición de negocio (ver vault Tienda): el pago y la logística no se mezclan.
  var ESTADOS_PAGO = ['pendiente', 'acreditado'];
  // Efecto de una transición de ESTADO (eje logístico) sobre el STOCK del producto:
  //   'baja'      = al ENTRAR a en_camino (el producto salió físicamente)
  //   'reingreso' = al SALIR de en_camino hacia cancelado / atrás (vuelve al depósito)
  //   'nada'      = resto (crear/confirmar NO reservan; entregado NO toca stock — ya bajó al salir)
  function efectoStock(estadoDe, estadoA) {
    if (estadoA === 'en_camino' && estadoDe !== 'en_camino') return 'baja';
    if (estadoDe === 'en_camino' && estadoA !== 'en_camino' && estadoA !== 'entregado') return 'reingreso';
    return 'nada';
  }
  // Venta consumada = los DOS ejes cerrados: estado 'entregado' Y pago 'acreditado'.
  function ventaConsumada(pedido) {
    return !!pedido && pedido.estado === 'entregado' && pedido.pago === 'acreditado';
  }
  ...
  return {
    ...
    efectoStock: efectoStock,
    ventaConsumada: ventaConsumada,
  };
});
