...
(function (root, factory) {
  ...
  function coberturaMascota(mascota) {
    ...
    var META = {
      activo: { label: 'Plan activo — cobertura vigente', color: '#166534', emoji: '✅' },
      suspendido: { label: 'Plan suspendido — sin cobertura', color: '#991b1b', emoji: '❌' },
      baja: { label: 'Dada de baja — sin cobertura', color: '#991b1b', emoji: '❌' },
    };
    ...
    return {
      ...
      color: m.color,
      emoji: m.emoji,
      chip: ok ? 'Activo' : (estado === 'baja' ? 'Baja' : 'Suspendido'),
    };
  }
  ...
  // Resumen de negocio para el dashboard admin: la unidad es la MASCOTA ACTIVA (no el titular).
  // facturacion = Σ cuota de mascotas activas. planCount = distribución por plan (solo activas).
  function resumenNegocio(mascotas) {
    var lista = mascotas || [];
    var activas = lista.filter(function (m) { return coberturaMascota(m).ok; });
    var planCount = {};
    activas.forEach(function (m) {
      var p = m.plan || 'Sin definir';
      planCount[p] = (planCount[p] || 0) + 1;
    });
    return {
      mascotasTotales: lista.length,
      mascotasActivas: activas.length,
      facturacion: activas.reduce(function (acc, m) { return acc + planCuota(m.plan); }, 0),
      planCount: planCount,
    };
  }
