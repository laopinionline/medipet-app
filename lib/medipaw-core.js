...
(function (root, factory) {
  ...
  // y las REGLAS Firestore la replican (mismo ANIO_MS y mismos cortes) para que el titular no pueda mandar valores
  // arbitrarios. Cortes perro/gato: Joven <1 año · Adulto 1–7 · Senior >7.
  ...
  // Mapeo perro/gato (conserva la lógica de franjas: <1→Joven, 1–7→Adulto, >7→Senior):
  ...
  function planPorEdadAprox(especie, edadAprox) {
    if (especie === 'perro' || especie === 'gato') {
      if (edadAprox === 'cachorro') return 'MEDIPaw Joven';
      if (edadAprox === 'mayor') return 'MEDIPaw Senior';
      return 'MEDIPaw Adulto'; // joven, adulto
    }
  ...
  function planPorEspecieEdad(especie, fnacMs, nowMs) {
    if (especie === 'perro' || especie === 'gato') {
      ...
      if (e < 1) return 'MEDIPaw Joven';
      if (e > 7) return 'MEDIPaw Senior';
      return 'MEDIPaw Adulto';
    }
  ...
  function franjaEtaria(especie, fnacMs, nowMs) {
    ...
    var e = edadAnios(fnacMs, nowMs);
    return e < 1 ? 'Joven' : (e > 7 ? 'Senior' : 'Adulto');
  }
   ...
   *
   * ⚠️ PARA CONFIRMAR CON LUCAS (asunciones; revisa entrada por entrada antes de Fase 2):
   *   C1. 'MEDIPaw Básico' (ave/otros, $40.000) NO está en la tabla comparativa → sin prestaciones acá (sin motor
   *       hasta confirmar qué cubre).
   *   C2. pct de reintegro cuando la landing da SOLO tope (consulta, análisis, imagen, etólogo, nutricionista,
   *       cremación): se asume PCT_PLENO = 1.0 (100% hasta el tope).
   *   C3. Carencia: se mide UNA VEZ desde el alta (no se re-aplica cada aniversario). CONFIRMAR contra R1.
   *   C4. URGENCIAS · cirugía/análisis/imagen: 'desde 60/90 días' figura solo en Joven/Adulto/Senior; en Urgencias
   *       no hay espera indicada → se asume carencia 0.
   *   C5. Sin tope explícito (medicamentos, vacunas, desparasitación, higiene, baño): tope null (solo %, sin límite $).
   *   C6. 'hasta 25% off' / 'hasta 10km': se toma el máximo como valor fijo (25% / 10km).
   *   C7. Traslado ('hasta 10km c/u'): cap de distancia, no reintegro en $ → pct/tope null + nota.
   *   C8. Cremación: cupo anual no indicado → se asume 1/año.
   *   C9. Beneficios de acceso (Vet online, Descuentos, Asesoramiento legal): sin cupo/tope/pct (ilimitado).
   * ═══════════════════════════════════════════════════════════════════════════════════════════ */
  var DIA_MS = 86400000;
  ...
  var PLANES_COB = ['MEDIPaw Urgencias', 'MEDIPaw Joven', 'MEDIPaw Adulto', 'MEDIPaw Senior']; // C1: Básico NO está.
  ...
  var COBERTURAS = {
    consulta: { nombre: 'Consultas y guardia 24hs', categoria: 'Consulta veterinaria',
      cupoAnual: 2, pct: PCT_PLENO /*C2*/, tope: 35000, carenciaDias: 0, postCupo: 'socio', nota: 'Subsiguientes con descuento',
      planes: { 'MEDIPaw Urgencias': {}, 'MEDIPaw Joven': {}, 'MEDIPaw Adulto': {}, 'MEDIPaw Senior': {} } },
    ...
    traslado: { nombre: 'Traslado con mascota', categoria: 'Cuidado regular', nota: 'hasta 10km c/u (C7)',
  ...
  // Alta en ms (contrato de las funciones puras): mascota.altaMs (number). El caller convierte creadoEn→ms.
  function _altaMs(m) { return (m && typeof m.altaMs === 'number') ? m.altaMs : null; }
  ...
  function carenciaCumplida(mascota, prestacion, fechaMs) {
    ...
    if (!r) return { cumplida: false, incluida: false, carenciaDias: null, desdeMs: null };
    var alta = _altaMs(mascota), dias = r.carenciaDias || 0;
    var desde = (alta != null) ? alta + dias * DIA_MS : null;
    return { cumplida: (desde == null) ? true : (Number(fechaMs) >= desde), incluida: true, carenciaDias: dias, desdeMs: desde };
  }
  ...
  function cobertura(mascota, prestacion, monto, contexto) {
    ...
    // 3) carencia (desde el alta)
    var car = carenciaCumplida(mascota, prestacion, fechaMs);
    if (!car.cumplida) return { cubre: false, pct: 0, tope: null, aCargoSocio: monto, reintegro: 0, motivo: 'En carencia hasta el ' + (car.desdeMs != null ? _fmtDM(car.desdeMs) : '—') };
    // 4) cupo del AÑO DE ANIVERSARIO vigente (R1): filtrar las atenciones a la ventana
    var vent = ventanaAniversario(_altaMs(mascota), fechaMs);
    var atsAnio = (contexto.atenciones || []).filter(function (a) {
      var t = Number(a && (a.fechaMs != null ? a.fechaMs : a.fecha)) || 0;
      return vent ? (t >= vent.inicio && t < vent.fin) : true;
    });
